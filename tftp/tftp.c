...
int tftp_send_ack(struct tftp_conn *tc)
{
	...
	ack->blocknr = tc->blocknr; //checar el numero de ceros 
				...
				if (retval == -1)
					perror("select()");
 					...
 					printf("Data is available now.\n");
               		...
               		if (FD_ISSET(tc->sock, &readfds))
					//printf("A block or akc was send!\n");
					if ((nbytes = recvfrom(tc->sock, bu, 800, 0, 
							(struct sockaddr *)&tc->peer_addr, tc->addrlen))) <= 0){
